...
export default function EnhancedDashboard() {
  ...
  // Xác định preset so sánh
  const comparePreset = useMemo(() => {
    if (['yesterday', 'last7days', 'last30days'].includes(dateRange)) return null;
    return 'yesterday';
  }, [dateRange]);

  // Load data với caching
  const { data: currentData, loading, refetch } = useAPI(
    `reports/dashboard?preset=${dateRange}`,
    { dependencies: [dateRange], skipCache: false }
  );

  const { data: compareData } = useAPI(
    comparePreset ? `reports/dashboard?preset=${comparePreset}` : null,
    { dependencies: [comparePreset], skipCache: false, autoFetch: !!comparePreset }
  );
  ...
  const data = useMemo(() => {
    ...
    const today = currentData.overall || {};
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Performance Chart - Always show with real data */}
      <PerformanceChart 
        title="Hiệu Suất 24h" 
        data={data?.chartData || []}
      />
          ...
          <div className="space-y-2.5">
            {topPerformers.length === 0 ? (
              ...
              topPerformers.map((campaign, index) => (
                <div
                  key={campaign.id}
                  className="flex items-center justify-between p-3 rounded-ios bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800"
                >
          ...
          <div className="space-y-2.5">
            {worstPerformers.length === 0 ? (
              ...
              worstPerformers.map((campaign, index) => (
                <div
                  key={campaign.id}
                  className="flex items-center justify-between p-3 rounded-ios bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800"
                >
                  <div className="flex items-center gap-3">
