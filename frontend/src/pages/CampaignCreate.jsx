...
import { Save, X, Target, Info, Search } from 'lucide-react';
...
export default function CampaignCreate() {
  ...
  const [formData, setFormData] = useState({
    name: '',
    traffic_source: '',
    flow_type: 'direct', // 'direct' or 'lander'
    landing_page_id: '',
    offer_url: '',
  ...
  // Load all required data với batching
  const { data, loading } = useBatchAPI([
  ...
  const customDomains = useMemo(() => 
    data['custom-domains/active']?.success ? (data['custom-domains/active'].domains || []) : []
  , [data]);

  const landingPages = useMemo(() => {
    const apiData = data['landing-pages'];
    return apiData ? (apiData.landers || apiData.Landers || apiData.data || []) : [];
  }, [data]);

  const trafficSources = useMemo(() => 
    data['traffic-sources']?.success ? (data['traffic-sources'].data || []) : []
  , [data]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (data.success) {
        // Clear campaigns cache so the list page shows the new campaign
        clearCacheFor('campaigns');
        // API returns campaign_id, not campaign.id
        navigate(`/campaigns/${data.campaign_id}`);
      } else {
        alert(data.message || 'Lỗi tạo chiến dịch');
      }
    } catch (error) {
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400">
            Thiết lập chiến dịch tracking mới
          </p>
        </div>
        <button 
      ...
      <div className="card-ios p-6">
        <form onSubmit={handleSubmit}>
          <div className="space-y-5">
            <div>
              <label htmlFor="create-campaign-name" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                Tên Chiến Dịch *
              </label>
              <input
            ...
            <div>
              <label htmlFor="create-campaign-traffic-source" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                Nguồn Lưu Lượng
              </label>
              <select
                id="create-campaign-traffic-source"
                name="traffic_source"
            ...
            <div>
              <label htmlFor="create-campaign-flow-type" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                Loại Flow Campaign *
              </label>
              <select
                id="create-campaign-flow-type"
                name="flow_type"
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                {formData.flow_type === 'direct' 
                  ? '🚀 Redirect trực tiếp từ traffic source đến offer (nhanh nhưng CR thấp hơn)'
                  : '📄 Dùng landing page làm trang đệm để pre-sell trước khi đến offer (CR cao hơn)'}
              </p>
            </div>

            {formData.flow_type === 'lander' && (
              <div>
                <label htmlFor="create-campaign-landing-page" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                  Chọn Landing Page *
                </label>
                <select
                  id="create-campaign-landing-page"
            ...
            <div>
              <label htmlFor="create-campaign-network" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                Affiliate Network
              </label>
            ...
            <div>
              <label htmlFor="create-campaign-offer-url" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                Offer URL *
              </label>
              <input
                id="create-campaign-offer-url"
                name="offer_url"
                type="url"
                autoComplete="url"
                required
            ...
            {selectedNetwork === 'custom' && (
              <div>
                <label htmlFor="create-campaign-click-param" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                  Tên Tham Số Click ID
                </label>
                <input
                  id="create-campaign-click-param"
                  name="custom_click_param"
                  type="text"
                  value={formData.custom_click_param}
            ...
            <div>
              <label htmlFor="create-campaign-custom-domain" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                Tên Miền *
              </label>
              <select
                id="create-campaign-custom-domain"
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label htmlFor="create-campaign-cost-model" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                  Mô Hình Chi Phí
                </label>
                <select
                  id="create-campaign-cost-model"
                  name="cost_model"
              ...
              <div>
                <label htmlFor="create-campaign-cost-value" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                  Giá Trị ($)
                </label>
                <input
                  id="create-campaign-cost-value"
                  name="cost_value"
                  type="number"
                  step="0.01"
                  autoComplete="off"
            ...
            <div>
              <label htmlFor="create-campaign-status" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                Trạng Thái
              </label>
              <select
          ...
          {currentNetworkInfo && (
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    className="w-full px-3 py-2 text-sm border border-gray-300 dark:border-gray-600 rounded-ios bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-white font-mono"
                  ...
                  <button
                    ...
                    className="absolute right-2 top-1/2 -translate-y-1/2 text-blue-500 hover:text-blue-600 text-xs"
                  >
                    Copy
                  </button>
                </div>
              ...
              <div>
                ...
                <div className="p-3 bg-gray-50 dark:bg-gray-800 rounded-ios text-sm font-mono text-gray-900 dark:text-white break-all">
                  {currentNetworkInfo.offerUrlTemplate}
                </div>
                <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                  Thay YOUR_* bằng thông tin thực từ {currentNetworkInfo.name}
                </p>
              </div>
              ...
              <div>
                ...
                <div className="p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-ios">
              ...
              {/* Important Note */}
              <div className="p-3 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-ios">
                <p className="text-sm text-yellow-800 dark:text-yellow-300">
                  ⚠️ <strong>Quan trọng:</strong> Parameter <code className="bg-yellow-100 dark:bg-yellow-900/50 px-1 rounded">{currentNetworkInfo.clickParam}</code> trong offer URL phải match với <code className="bg-yellow-100 dark:bg-yellow-900/50 px-1 rounded">{`{${currentNetworkInfo.clickParam}}`}</code> trong postback URL!
                </p>
              </div>
            </div>
          )}
